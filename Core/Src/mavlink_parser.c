/**
 ******************************************************************************
 * @file           : mavlink_parser.c
 * @brief          : MAVLINK PX4 Protocol Parser Implementation
 ******************************************************************************
 * @attention
 *
 * MAVLINK v1 Parser Implementation
 * Based on YouXiang optical flow sensor reference code
 *
 ******************************************************************************
 */

#include "mavlink_parser.h"
#include <string.h>

/* CRC Extra Bytes for Message IDs (from MAVLINK specification) */
#define MAVLINK_CRC_EXTRA_OPTICAL_FLOW_RAD                                     \
  138 // Message ID 0x6A (106) - YouXiang specific
#define MAVLINK_CRC_EXTRA_DISTANCE_SENSOR 85 // Message ID 0x84 (132)

/**
 * @brief CRC-16-MCRF4XX lookup table
 */
static const uint16_t crc_table[256] = {
    0x0000, 0x1189, 0x2312, 0x329b, 0x4624, 0x57ad, 0x6536, 0x74bf, 0x8c48,
    0x9dc1, 0xaf5a, 0xbed3, 0xca6c, 0xdbe5, 0xe97e, 0xf8f7, 0x1081, 0x0108,
    0x3393, 0x221a, 0x56a5, 0x472c, 0x75b7, 0x643e, 0x9cc9, 0x8d40, 0xbfdb,
    0xae52, 0xdaed, 0xcb64, 0xf9ff, 0xe876, 0x2102, 0x308b, 0x0210, 0x1399,
    0x6726, 0x76af, 0x4434, 0x55bd, 0xad4a, 0xbcc3, 0x8e58, 0x9fd1, 0xeb6e,
    0xfae7, 0xc87c, 0xd9f5, 0x3183, 0x200a, 0x1291, 0x0318, 0x77a7, 0x662e,
    0x54b5, 0x453c, 0xbdcb, 0xac42, 0x9ed9, 0x8f50, 0xfbef, 0xea66, 0xd8fd,
    0xc974, 0x4204, 0x538d, 0x6116, 0x709f, 0x0420, 0x15a9, 0x2732, 0x36bb,
    0xce4c, 0xdfc5, 0xed5e, 0xfcd7, 0x8868, 0x99e1, 0xab7a, 0xbaf3, 0x5285,
    0x430c, 0x7197, 0x601e, 0x14a1, 0x0528, 0x37b3, 0x263a, 0xdecd, 0xcf44,
    0xfddf, 0xec56, 0x98e9, 0x8960, 0xbbfb, 0xaa72, 0x6306, 0x728f, 0x4014,
    0x519d, 0x2522, 0x34ab, 0x0630, 0x17b9, 0xef4e, 0xfec7, 0xcc5c, 0xddd5,
    0xa96a, 0xb8e3, 0x8a78, 0x9bf1, 0x7387, 0x620e, 0x5095, 0x411c, 0x35a3,
    0x242a, 0x16b1, 0x0738, 0xffcf, 0xee46, 0xdcdd, 0xcd54, 0xb9eb, 0xa862,
    0x9af9, 0x8b70, 0x8408, 0x9581, 0xa71a, 0xb693, 0xc22c, 0xd3a5, 0xe13e,
    0xf0b7, 0x0840, 0x19c9, 0x2b52, 0x3adb, 0x4e64, 0x5fed, 0x6d76, 0x7cff,
    0x9489, 0x8500, 0xb79b, 0xa612, 0xd2ad, 0xc324, 0xf1bf, 0xe036, 0x18c1,
    0x0948, 0x3bd3, 0x2a5a, 0x5ee5, 0x4f6c, 0x7df7, 0x6c7e, 0xa50a, 0xb483,
    0x8618, 0x9791, 0xe32e, 0xf2a7, 0xc03c, 0xd1b5, 0x2942, 0x38cb, 0x0a50,
    0x1bd9, 0x6f66, 0x7eef, 0x4c74, 0x5dfd, 0xb58b, 0xa402, 0x9699, 0x8710,
    0xf3af, 0xe226, 0xd0bd, 0xc134, 0x39c3, 0x284a, 0x1ad1, 0x0b58, 0x7fe7,
    0x6e6e, 0x5cf5, 0x4d7c, 0xc60c, 0xd785, 0xe51e, 0xf497, 0x8028, 0x91a1,
    0xa33a, 0xb2b3, 0x4a44, 0x5bcd, 0x6956, 0x78df, 0x0c60, 0x1de9, 0x2f72,
    0x3efb, 0xd68d, 0xc704, 0xf59f, 0xe416, 0x90a9, 0x8120, 0xb3bb, 0xa232,
    0x5ac5, 0x4b4c, 0x79d7, 0x685e, 0x1ce1, 0x0d68, 0x3ff3, 0x2e7a, 0xe70e,
    0xf687, 0xc41c, 0xd595, 0xa12a, 0xb0a3, 0x8238, 0x93b1, 0x6b46, 0x7acf,
    0x4854, 0x59dd, 0x2d62, 0x3ceb, 0x0e70, 0x1ff9, 0xf78f, 0xe606, 0xd49d,
    0xc514, 0xb1ab, 0xa022, 0x92b9, 0x8330, 0x7bc7, 0x6a4e, 0x58d5, 0x495c,
    0x3de3, 0x2c6a, 0x1ef1, 0x0f78};

/**
 * @brief Accumulate one byte into CRC-16 checksum
 * @param data: Data byte
 * @param crcAccum: Accumulated CRC value
 * @retval Updated CRC value
 */
static inline void crc_accumulate(uint8_t data, uint16_t *crcAccum) {
  uint8_t tmp = data ^ (uint8_t)(*crcAccum & 0xff);
  *crcAccum = (*crcAccum >> 8) ^ crc_table[tmp];
}

/**
 * @brief Compute CRC-16-MCRF4XX checksum for MAVLINK
 * @param data: Pointer to data buffer
 * @param length: Length of data
 * @param crc_extra: CRC extra byte for message ID
 * @retval Computed CRC-16 checksum
 */
uint16_t mavlink_crc_compute(const uint8_t *data, uint16_t length,
                             uint8_t crc_extra) {
  uint16_t crc = 0xFFFF; // Initial value

  // Accumulate data bytes
  for (uint16_t i = 0; i < length; i++) {
    crc_accumulate(data[i], &crc);
  }

  // Accumulate CRC extra byte
  crc_accumulate(crc_extra, &crc);

  return crc;
}

/**
 * @brief Initialize MAVLINK parser
 * @param parser: Pointer to parser context
 */
void mavlink_parser_init(mavlink_parser_t *parser) {
  memset(parser, 0, sizeof(mavlink_parser_t));
  parser->state = MAVLINK_PARSE_STATE_IDLE;
}

/**
 * @brief Reset parser to idle state
 * @param parser: Pointer to parser context
 */
static void parser_reset(mavlink_parser_t *parser) {
  parser->state = MAVLINK_PARSE_STATE_IDLE;
  parser->payload_idx = 0;
  parser->checksum = 0xFFFF;
}

/**
 * @brief Parse optical flow message payload
 * @param parser: Pointer to parser context
 */
static void parse_optical_flow(mavlink_parser_t *parser) {
  memcpy(&parser->optical_flow, parser->payload,
         sizeof(mavlink_optical_flow_rad_t));
  parser->optical_flow_ready = true;
}

/**
 * @brief Parse distance sensor message payload
 * @param parser: Pointer to parser context
 */
static void parse_distance_sensor(mavlink_parser_t *parser) {
  memcpy(&parser->distance_sensor, parser->payload,
         sizeof(mavlink_distance_sensor_t));
  parser->distance_ready = true;
}

/**
 * @brief Parse a single byte from MAVLINK stream
 * @param parser: Pointer to parser context
 * @param byte: Byte to parse
 * @retval true if a complete message was received
 */
bool mavlink_parse_char(mavlink_parser_t *parser, uint8_t byte) {
  bool message_received = false;

  switch (parser->state) {
  case MAVLINK_PARSE_STATE_IDLE:
    if (byte == MAVLINK_STX) {
      parser->checksum = 0xFFFF;
      parser->payload_idx = 0;
      parser->state = MAVLINK_PARSE_STATE_GOT_STX;
    }
    break;

  case MAVLINK_PARSE_STATE_GOT_STX:
    parser->payload_len = byte;
    crc_accumulate(byte, &parser->checksum);

    // ï¿?? Validate payload length doesn't exceed maximum allowed by protocol
    if (parser->payload_len > MAVLINK_MAX_PAYLOAD_LEN) {
      // Invalid payload length, frame is corrupted - reset parser
      parser_reset(parser);
    } else {
      parser->state = MAVLINK_PARSE_STATE_GOT_LENGTH;
    }
    break;

  case MAVLINK_PARSE_STATE_GOT_LENGTH:
    parser->seq = byte;
    crc_accumulate(byte, &parser->checksum);
    parser->state = MAVLINK_PARSE_STATE_GOT_SEQ;
    break;

  case MAVLINK_PARSE_STATE_GOT_SEQ:
    parser->sysid = byte;
    crc_accumulate(byte, &parser->checksum);
    parser->state = MAVLINK_PARSE_STATE_GOT_SYSID;
    break;

  case MAVLINK_PARSE_STATE_GOT_SYSID:
    parser->compid = byte;
    crc_accumulate(byte, &parser->checksum);
    parser->state = MAVLINK_PARSE_STATE_GOT_COMPID;
    break;

  case MAVLINK_PARSE_STATE_GOT_COMPID:
    parser->msgid = byte;
    crc_accumulate(byte, &parser->checksum);

    // Early validation: verify message ID and payload length match
    bool valid_message = false;
    if ((parser->msgid == MAVLINK_MSG_ID_OPTICAL_FLOW_RAD &&
         parser->payload_len == MAVLINK_OPTICAL_FLOW_RAD_LEN) ||
        (parser->msgid == MAVLINK_MSG_ID_DISTANCE_SENSOR &&
         parser->payload_len == MAVLINK_DISTANCE_SENSOR_LEN) ||
        parser->payload_len == 0) {
      valid_message = true;
    }

    if (!valid_message) {
      // Unknown message type or length mismatch - discard frame
      parser_reset(parser);
    } else if (parser->payload_len == 0) {
      parser->state = MAVLINK_PARSE_STATE_GOT_PAYLOAD;
    } else {
      parser->state = MAVLINK_PARSE_STATE_GOT_MSGID;
    }
    break;

  case MAVLINK_PARSE_STATE_GOT_MSGID:
    //  Buffer overflow protection: check payload index boundary
    if (parser->payload_idx < MAVLINK_MAX_PAYLOAD_LEN) {
      parser->payload[parser->payload_idx++] = byte;
      crc_accumulate(byte, &parser->checksum);

      if (parser->payload_idx >= parser->payload_len) {
        parser->state = MAVLINK_PARSE_STATE_GOT_PAYLOAD;
      }
    } else {
      // Payload too long (>255 bytes), discard this frame and reset parser
      // This prevents buffer overflow that would corrupt optical_flow struct
      parser_reset(parser);
    }
    break;

  case MAVLINK_PARSE_STATE_GOT_PAYLOAD:
    parser->crc_low = byte;
    parser->state = MAVLINK_PARSE_STATE_GOT_CRC1;
    break;

  // ================= CRC Check =================
  case MAVLINK_PARSE_STATE_GOT_CRC1: { // <----
                                       // CRC high byte received, now validate
    parser->crc_high = byte;

    // Add CRC extra byte based on message ID
    uint8_t crc_extra = 0;
    bool valid_msg = false;

    if (parser->msgid == MAVLINK_MSG_ID_OPTICAL_FLOW_RAD &&
        parser->payload_len == MAVLINK_OPTICAL_FLOW_RAD_LEN) {
      crc_extra = MAVLINK_CRC_EXTRA_OPTICAL_FLOW_RAD;
      valid_msg = true;
    } else if (parser->msgid == MAVLINK_MSG_ID_DISTANCE_SENSOR &&
               parser->payload_len == MAVLINK_DISTANCE_SENSOR_LEN) {
      crc_extra = MAVLINK_CRC_EXTRA_DISTANCE_SENSOR;
      valid_msg = true;
    }

    if (valid_msg) {
      // Accumulate CRC extra byte
      crc_accumulate(crc_extra, &parser->checksum);

      // Verify checksum
      uint16_t received_crc =
          (uint16_t)parser->crc_low | ((uint16_t)parser->crc_high << 8);
      if (received_crc == parser->checksum) {
        // Valid message - parse payload
        if (parser->msgid == MAVLINK_MSG_ID_OPTICAL_FLOW_RAD) {
          parse_optical_flow(parser);
          message_received = true;
        } else if (parser->msgid == MAVLINK_MSG_ID_DISTANCE_SENSOR) {
          parse_distance_sensor(parser);
          message_received = true;
        }
      }
    }

    // Reset parser for next message
    parser_reset(parser);
  } break;
    // ===============================================

  default:
    parser_reset(parser);
    break;
  }

  return message_received;
}

/**
 * @brief Get optical flow data if available
 * @param parser: Pointer to parser context
 * @param flow: Pointer to optical flow structure to fill
 * @retval true if new data is available
 */
bool mavlink_get_optical_flow(mavlink_parser_t *parser,
                              mavlink_optical_flow_rad_t *flow) {
  if (parser->optical_flow_ready) {
    memcpy(flow, &parser->optical_flow, sizeof(mavlink_optical_flow_rad_t));
    parser->optical_flow_ready = false;
    return true;
  }
  return false;
}

/**
 * @brief Get distance sensor data if available
 * @param parser: Pointer to parser context
 * @param distance: Pointer to distance sensor structure to fill
 * @retval true if new data is available
 */
bool mavlink_get_distance(mavlink_parser_t *parser,
                          mavlink_distance_sensor_t *distance) {
  if (parser->distance_ready) {
    memcpy(distance, &parser->distance_sensor,
           sizeof(mavlink_distance_sensor_t));
    parser->distance_ready = false;
    return true;
  }
  return false;
}
