...
static uint8_t ano_distance_tx_buf[ANO_HEADER_LEN + ANO_DISTANCE_DATA_LEN + ANO_CHECKSUM_LEN];
static uint8_t ano_flow_tx_buf[ANO_HEADER_LEN + ANO_FLOW_MODE1_DATA_LEN + ANO_CHECKSUM_LEN];
 ...
 */
static uint8_t ano_build_frame(uint8_t frame_id, const uint8_t *data, uint8_t data_len, uint8_t *buffer)
{
    uint8_t idx = 0;
...
{
    uint8_t frame_buffer[ANO_HEADER_LEN + ANO_DISTANCE_DATA_LEN + ANO_CHECKSUM_LEN];
    uint8_t data_buffer[ANO_DISTANCE_DATA_LEN];

    // Pack distance data into buffer
    data_buffer[0] = distance_data->direction;
    data_buffer[1] = distance_data->angle;
    data_buffer[2] = (uint8_t)(distance_data->distance_mm & 0xFF);        // Distance low byte
    data_buffer[3] = (uint8_t)((distance_data->distance_mm >> 8) & 0xFF); // Distance high byte
    data_buffer[4] = distance_data->status;
    data_buffer[5] = distance_data->reserved1;
    data_buffer[6] = distance_data->reserved2;

    // Build complete frame
    uint8_t frame_len = ano_build_frame(ANO_FRAME_ID_DISTANCE, data_buffer, ANO_DISTANCE_DATA_LEN, frame_buffer);
...
{
    uint8_t frame_buffer[ANO_HEADER_LEN + ANO_FLOW_MODE1_DATA_LEN + ANO_CHECKSUM_LEN];
    uint8_t data_buffer[ANO_FLOW_MODE1_DATA_LEN];

    // Pack optical flow data into buffer
    data_buffer[0] = flow_data->mode;       // Mode (should be 1 for Mode 1)
    data_buffer[1] = flow_data->quality;    // Quality (0-255)
    data_buffer[2] = (uint8_t)(flow_data->dx & 0xFF);        // DX low byte
    data_buffer[3] = (uint8_t)((flow_data->dx >> 8) & 0xFF); // DX high byte
    data_buffer[4] = (uint8_t)(flow_data->dy & 0xFF);        // DY low byte
    data_buffer[5] = (uint8_t)((flow_data->dy >> 8) & 0xFF); // DY high byte
    data_buffer[6] = flow_data->reserved;   // Reserved

    // Build complete frame
    uint8_t frame_len = ano_build_frame(ANO_FRAME_ID_OPTICAL_FLOW, data_buffer, ANO_FLOW_MODE1_DATA_LEN, frame_buffer);
...
HAL_StatusTypeDef ano_send_distance_dma(UART_HandleTypeDef *huart, const ano_distance_data_t *distance_data)
{
    ...
    data_buffer[0] = distance_data->direction;
    data_buffer[1] = distance_data->angle;
    data_buffer[2] = (uint8_t)(distance_data->distance_mm & 0xFF);
    data_buffer[3] = (uint8_t)((distance_data->distance_mm >> 8) & 0xFF);
    data_buffer[4] = distance_data->status;
    data_buffer[5] = distance_data->reserved1;
    data_buffer[6] = distance_data->reserved2;

    // Build complete frame into static buffer
    uint8_t frame_len = ano_build_frame(ANO_FRAME_ID_DISTANCE, data_buffer, ANO_DISTANCE_DATA_LEN, ano_distance_tx_buf);
...
HAL_StatusTypeDef ano_send_optical_flow_mode1_dma(UART_HandleTypeDef *huart, const ano_optical_flow_mode1_t *flow_data)
{
    ...
    // Pack optical flow data into buffer
    data_buffer[0] = flow_data->mode;
    data_buffer[1] = flow_data->quality;
    ...
    data_buffer[6] = flow_data->reserved;

    // Build complete frame into static buffer
    uint8_t frame_len = ano_build_frame(ANO_FRAME_ID_OPTICAL_FLOW, data_buffer, ANO_FLOW_MODE1_DATA_LEN, ano_flow_tx_buf);
